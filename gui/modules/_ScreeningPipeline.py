...
from ccpn.AnalysisScreen.lib.__demoBrukerSpectrum import writeBruker, createStdDifferenceSpectrum, matchedPosition
    ...
    for sample in self.project.samples:
      if not sample.isVirtual:
        for spectrum in sample.spectra:
         spectrum.peakLists[0].pickPeaks1dFiltered(size=9, excludeRegions=None, positiveNoiseThreshold=0)
        for sampleComponent in sample.sampleComponents:
          sampleComponent.substance.referenceSpectra[0].peakLists[0].pickPeaks1dFiltered(excludeRegions=None, positiveNoiseThreshold=0)
